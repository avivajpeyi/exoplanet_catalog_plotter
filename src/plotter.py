...
def plot_radii_hist(ax):
    ...
    cts, b, _ = ax.hist(
        c.pl_rade, bins=bins, density=False, histtype="step", lw=3, color="gray"
    )
...
def plot_radius_mass_relation_plot(ax):
    ...
    yl = 0.4
    kw = dict(
        ha="right", va="bottom", xycoords="data", rotation=90, fontsize=fs, color="gray"
    )
    ...
    kw = dict(
        ha="center", va="bottom", xycoords="data", fontsize=fs, color="navy", alpha=1
    )
...
def get_luminosity_ratio(star_radius_km, star_temp_k):
    ...
    """
    return np.power(arg_star_km / R_SUN, 2) * np.power(arg_star_temp / T_SUN, 4)
